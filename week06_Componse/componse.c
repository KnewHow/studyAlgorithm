...
/*
 * Use DFS method to list the component of a graph ordered by the index in
 * adjacent matrix
 * @param graph The graph need to list the component
 * @param startPoint The start point we start search the tree
 * @paran int *visited The array we use to tag the vertex we has visited.
 */
void DFSComponents(adjacentMatrixGraph graph, vertex startPoint, int *visited) {
	int i;
	for (i = 0; i < graph->vertex_number; i++) {
		if (visited[i] == 0) {
			printf("{ ");
...
/*
 * Use BFS method to list the component of a graph ordered by the index
 * adjacent matrix
 */
void BFSComponents(adjacentMatrixGraph graph, vertex startPoint, int *visited) {
	int i;
	for (i = 0; i < graph->vertex_number; i++) {
		if (visited[i] == 0) {
			printf("{ ");
...
int main() {
	...
	DFSComponents(graph, 0, visited);
	initVisited(graph, visited);
	BFSComponents(graph, 0, visited);
	return 0;
}
